...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Modal tutar inputu formatlaması
    const editAmountInput = document.getElementById('editCheckAmount');
    ...
    document.getElementById('editCheckDate').addEventListener('keypress', function (e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            const editAmount = document.getElementById('editCheckAmount');
            editAmount.focus();
            editAmount.select();
        }
    });

    // Modal dışına tıklayınca kapat-2 (İptal)
...
function closeEditModal() {
    ...
    document.getElementById('editModal').dataset.editingId = '';
}
...
function removeCheck(id) {
    ...
    document.getElementById('deleteModalInfo').innerHTML =
        `Çek No: <span>#${displayIndex}</span><br> Vade: <span>${formatDate(check.dueDate)}</span> &nbsp;·&nbsp; Tutar: <span>${formatMoney(check.amount)} TL</span>`;

    document.getElementById('deleteModal').dataset.deletingId = id;
...
function closeDeleteModal() {
    ...
    document.getElementById('deleteModal').dataset.deletingId = '';
}

// Silmeyi onayla
function confirmDelete() {
    const id = parseInt(document.getElementById('deleteModal').dataset.deletingId);
